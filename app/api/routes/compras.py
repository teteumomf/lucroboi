...
from app.domain.services.compra_service import criar_compra, listar_compras, buscar_compra, excluir_compra
...
def listar(
    ...
    return listar_compras(db=db, usuario=usuario)
...
def detalhe(
    ...
    try:
        return buscar_compra(db=db, compra_id=compra_id, usuario=usuario)
    except DomainError as e:
...
def excluir(
    ...
    try:
        excluir_compra(db=db, compra_id=compra_id, usuario=usuario)
    except DomainError as e:
        raise HTTPException(status_code=400, detail=str(e))
